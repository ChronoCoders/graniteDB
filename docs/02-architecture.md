...
- WAL (Write-Ahead Log)
- MemTable (mutable)
- Immutable MemTables (flush queue, planned)
- SSTables (disk)
- VersionSet (in-memory metadata state)
- Manifest (persistent metadata log)
- Compaction engine (background, planned)
...
## 2.5 Current Status (Prototype)

The current implementation keeps the model simple:

- No immutable memtable queue yet (a single MemTable is flushed).
- Flush/compaction are triggered synchronously from the writer path (under the writer lock).
